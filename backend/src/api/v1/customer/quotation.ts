...
quotation.post("/",
    ...
    (req: UserRequest, res) => {
        ...
        db.beginTransaction((err) => {
            ...
            db.query('INSERT INTO Quote (propertyId, userId, name, contactNumber, email, address, country, state, remodelingDate, zipcode, specialRequest) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [propertyId, req.userId, name, phone, email, address, country, state, remodelingDate, zipcode, specialRequest], (err, result) => {
                if (err) {
                    console.log(err);
                    return res.status(500).json({ message: "Internal server error" });
                }

                db.query('SELECT * FROM Quote WHERE id = ?', [result.insertId], (err, result) => {
                    if (err) {
                        db.rollback((err) => {
                            if (err) {
                                console.error(err);
...
quotation.get("/raised/:quoteId", (req: UserRequest, res) => {
    ...
    const sql = "SELECT q.quoteId, q.createdDate, q.propertyId, up.length, up.breadth, up.height, up.area, up.doors, up.windows, up.budget, up.issues, up.objective, up.style, up.timeline, up.specialRequest, q.address FROM Quote as q INNER JOIN UserProperty as up WHERE q.quoteId = ? AND q.propertyId = up.id";
