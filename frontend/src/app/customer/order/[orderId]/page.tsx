...
import CustomerProfileStatusTrackingDialog from "@/components/custom/customer/CustomerProfileStatusTrackingDialog";
...
export default function OrderDetailsCS({ params: { orderId } }: Readonly<{ params: { orderId: string } }>) {
    const [trackingDialogOpen, setTrackingDialogOpen] = useState(false);

    return (
        <section>
            <Link href={"/customer/order"} className={cn('ml-auto block w-fit mb-4')}>
                <Button variant={"link"}>
                    <IoArrowBack className='mr-2 text-lg' />
                    <span title='back'>BACK</span>
                </Button>
            </Link>
            <CustomerProfileStatusTrackingDialog open={trackingDialogOpen} onOpenChange={() => {
