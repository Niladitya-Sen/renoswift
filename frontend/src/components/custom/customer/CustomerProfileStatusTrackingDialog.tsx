...
export default function CustomerProfileStatusTrackingDialog({ open, onOpenChange }: Readonly<{ open: boolean, onOpenChange: (open: boolean) => void }>) {
    return (
                ...
                <div className="mt-4 grid grid-cols-[1fr_0.3fr_1fr] mx-auto max-h-[calc(100svh-10rem)] overflow-y-auto">
                    {data.map((item, index) => (
                        <React.Fragment key={item.id}>
                            <p className="font-medium justify-self-end">{item.date}</p>
